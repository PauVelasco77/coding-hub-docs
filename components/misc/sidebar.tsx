    ...
    if (node.type === "file") {
      ...
      return (
        <li key={node.path} className='py-1'>
          <Link
            href={`/${node.slug}`}
            className={`block pl-${level * 4 + 4} hover:bg-gray-100 py-1 rounded ${
              isActive ? "bg-gray-100 font-medium text-blue-600" : "text-gray-700"
            }`}>
            {displayName}
          </Link>
        </li>
      ...
      const displayName = node.name.replace(/^\d+\s*-\s*/, ""); // Remove leading numbers and dashes
      ...
      return (
        <li key={node.path} className='py-1'>
          ...
          {isExpandedDir && <ul className='mt-1'>{node.children?.map((child) => renderNode(child, level + 1))}</ul>}
        </li>
      );
    }
  ...
  return (
    <div className={`sidebar bg-white h-full overflow-y-auto ${isOpen ? "block" : "hidden md:block"} ${className}`}>
      <div className='p-4'>
        ...
        <ul className='space-y-1'>{structure.children.map((node) => renderNode(node))}</ul>
      </div>
    </div>
  );
};
